...
class MusicPlayer extends FlxGroup 
{
	...
	public function new(instance:FreeplayState)
	{
		...
		songTxt.setFormat(Paths.font("upheavtt.ttf"), 32, FlxColor.WHITE, RIGHT);
		add(songTxt);

		timeTxt = new FlxText(xPos, songTxt.y + 60, 0, "", 32);
		timeTxt.setFormat(Paths.font("upheavtt.ttf"), 32, FlxColor.WHITE, RIGHT);
		add(timeTxt);

		for (i in 0...2)
		{
			var text:FlxText = new FlxText();
			text.setFormat(Paths.font('upheavtt.ttf'), 32, FlxColor.WHITE, CENTER);
			text.text = '^';
			if (i == 1)
				text.flipY = true;
			text.visible = false;
		...
		playbackTxt.setFormat(Paths.font("upheavtt.ttf"), 32, FlxColor.WHITE);
		add(playbackTxt);
