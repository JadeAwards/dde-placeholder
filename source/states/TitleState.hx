...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		ddeLogo = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('DDELogo'));
		ddeLogo.visible = false;
		ddeLogo.scale.set(0.4,0.4);
		ddeLogo.updateHitbox();
		ddeLogo.screenCenter(X);
		ddeLogo.antialiasing = false;
		...
		if(!closedState)
		{
			...
			switch (sickBeats)
			{
				...
				case 5:
					deleteCoolText();
				case 6:
					createCoolText(['Modjam', 'for'], -40);
				case 8:
					addMoreText('DDE', -40);
					ddeLogo.visible = true;
				case 9:
					deleteCoolText();
					ddeLogo.visible = false;
				case 10:
