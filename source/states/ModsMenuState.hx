...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if(modsList.all.length < 1)
		{
			...
			noModsTxt.setFormat(Paths.font("upheavtt.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			noModsTxt.borderSize = 2;
			add(noModsTxt);
			noModsTxt.screenCenter(Y);

			var txt = new FlxText(bgList.x + 15, bgList.y + 15, bgList.width - 30, "No Mods found.", 16);
			txt.setFormat(Paths.font("upheavtt.ttf"), 16, FlxColor.WHITE);
			add(txt);
		...
		modDesc.setFormat(Paths.font("upheavtt.ttf"), 24, FlxColor.WHITE, LEFT);
		add(modDesc);
		...
		modRestartText.setFormat(Paths.font("upheavtt.ttf"), 16, FlxColor.WHITE, RIGHT);
		add(modRestartText);
	...
	override function update(elapsed:Float)
	{
		...
		{
			if(controls.controllerMode) FlxG.mouse.visible = true;
			_lastControllerMode = controls.controllerMode;
		}
...
class ModItem extends FlxSpriteGroup
{
	...
	public function new(folder:String)
	{
		...
		text.setFormat(Paths.font("upheavtt.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		text.borderSize = 2;
